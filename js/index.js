...
function normalizeSource(value) {
    // 兼容旧收藏：apple → qq（Apple Music已替换为QQ音乐）
    if (value === "apple") return "qq";
...
const SOURCE_QUALITY_MAP = {
    ...
    kugou: ['999', '320', '128'],  // 酷狗免费音质：FLAC无损/320kbps MP3/128kbps MP3
    qq: ['999', '320', '128'],  // QQ音乐：FLAC无损/320kbps MP3/128kbps MP3
...
// 酷我原生歌词缓存：getSongUrlByRid 获取到 lyric 后缓存，供 getLyricWithFallback 复用
const kuwoLyricCache = new Map();
...
const API_KUWO = {
    ...
    search: async (keyword, count = 20) => {
        const url = `${API_KUWO.baseUrl}?key=${API_KUWO.key}&msg=${encodeURIComponent(keyword)}&g=${Math.min(count, 50)}&action=so`;
        ...
        try {
            ...
            debugLog(`[酷我API] 搜索响应: ${JSON.stringify(data).substring(0, 200)}...`);
            ...
            return data.data.songs.map(song => ({
                ...
                artist: song.singer,
                album: song.album,
                // API新增 image 字段：直接使用封面图直链，避免额外请求
                pic_url: song.image || "",
                pic_id: song.rid,
                url_id: song.rid,
                lyric_id: song.rid,
            ...
            debugLog(`[酷我API] 搜索错误: ${error.message}`);
            return [];
        }
    },
    ...
    getSongUrlByKeyword: async (keyword, index = 1, quality = "320") => {
        ...
        const kuwoQuality = qualityMap[quality] || 'SQ'; // 999(hires)/740(lossless)需VIP→降级SQ
        
        const url = `${API_KUWO.baseUrl}?key=${API_KUWO.key}&msg=${encodeURIComponent(keyword)}&n=${index}&size=${kuwoQuality}`;
        ...
        try {
            ...
            if (data.code === 200 && data.data && data.data.vipmusic && data.data.vipmusic.url) {
                return {
                    url: data.data.vipmusic.url,
                    br: data.data.vipmusic.bitrate,
                    size: 0,
                    apiSource: "kuwo_api",
                };
            }
    ...
    getSongUrlByRid: async (rid, quality = "320", msg = "") => {
        ...
        try {
            ...
            const data = await response.json();
            debugLog(`[酷我API] RID播放响应: ${JSON.stringify(data).substring(0, 200)}...`);
            ...
            if (data.code === 200 && data.data && data.data.vipmusic && data.data.vipmusic.url) {
                ...
                // 缓存原生歌词供 getLyricWithFallback 使用
                const lrcText = data.data.lyric && data.data.lyric.lrc;
                if (lrcText) {
                    kuwoLyricCache.set(String(rid), lrcText);
                }
                return result;
            }
            ...
            debugLog(`[酷我API] RID获取播放链接错误: ${error.message}`);
            return null;
        }
    },
    ...
    // 获取封面图片
    getPicUrl: (rid) => {
        return `http://artistpicserver.kuwo.cn/pic.web?rid=${rid}`;
    },
};
...
const API_KUGOU = {
    ...
    // 搜索歌曲
    search: async (keyword, count = 20) => {
        const url = `${API_KUGOU.baseUrl}?key=${API_KUGOU.key}&msg=${encodeURIComponent(keyword)}&g=${Math.min(count, 50)}`;
        ...
        try {
            ...
            const data = await response.json();
            debugLog(`[酷狗API] 搜索响应: ${JSON.stringify(data).substring(0, 200)}...`);
            
            if (data.code !== 200 || !data.data || !data.data.songs) {
                return [];
            }
            
            return data.data.songs.map((song, index) => ({
                id: song.name + "_" + song.singer, // 使用歌名+歌手作为ID
                name: song.name,
                artist: song.singer,
                album: song.album,
                pic_id: index.toString(),
            ...
            debugLog(`[酷狗API] 搜索错误: ${error.message}`);
            return [];
        }
    },
    ...
    // 获取封面图片
    getPicUrl: (songName, singer) => {
        // 酷狗封面需要从播放接口获取，这里返回一个通用占位
        return null;
    },
};

// QQ音乐API配置（替换原Apple Music）
const API_QQ = {
    ...
    search: async (keyword, count = 20) => {
        ...
        try {
            ...
            return data.data.songs
                ...
                .map((song, index) => ({
                    id: song.mid || (song.name + "_" + song.singer),
                    name: song.name,
                    artist: song.singer,
                    album: song.album,
                    pic_id: song.mid || index.toString(),
                    url_id: song.mid || (song.name + "_" + song.singer),
                    lyric_id: song.mid || (song.name + "_" + song.singer),
                    source: "qq",
                    apiSource: "qq_api",
                    // QQ音乐搜索不返回封面，留空由getPicUrl处理
                    pic_url: "",
                    // 保存原始搜索关键词和序号，供getSongUrlByIndex精确匹配
                    searchKeyword: keyword,
                    searchN: song.n,
                }));
        } catch (error) {
    ...
    getSongUrlByIndex: async (keyword, index = 1, quality = "128") => {
        ...
        try {
            ...
            const data = await response.json();
            debugLog(`[QQ音乐API] 单曲响应: ${JSON.stringify(data).substring(0, 200)}...`);

            if (data.code === 200 && data.data && data.data.musicurl) {
                return {
                    url: data.data.musicurl,
                    br: quality === 'flac' ? 999 : (quality === '320' ? 320 : 128),
                    size: 0,
                    apiSource: "qq_api",
                    picture: data.data.picture || "",
                };
...
const API = {
    ...
    search: async (keyword, source = "netease", count = 20, page = 1) => {
        ...
        // QQ音乐使用新的API（兼容旧收藏source=apple）
        if (source === "qq" || source === "apple") {
    ...
    getSongUrl: (song, quality = "999") => {
        // 喜马拉雅/QQ音乐使用各自的API获取音频，不走GD Studio
        if (song.source === "xima" || song.source === "qq" || song.source === "apple") {
            if (song.source === "xima") {
            ...
            // QQ音乐：走代理获取播放链接
            if (song.source === "qq" || song.source === "apple") {
                const qqQualityMap = { '999': 'flac', '320': '320', '128': '128' };
                const qqQuality = qqQualityMap[quality] || '128';
                const keyword = song.searchKeyword || song.name;
                const n = song.searchN || 1;
                const apiUrl = `https://api.yaohud.cn/api/music/qq?key=${API_QQ.key}&msg=${encodeURIComponent(keyword)}&n=${n}&size=${qqQuality}`;
                return `/proxy?target=${encodeURIComponent(apiUrl)}`;
            }
        }
    ...
    getPicUrl: (song) => {
        // 如果已有封面URL（妖狐API返回的picture/cover直链），走代理规避CDN防盗链
        if (song.pic_url) return buildAudioProxyUrl(song.pic_url);
    ...
    getAudioUrlWithFallback: async (song, quality = "320") => {
        ...
        if (song.source === "kuwo") {
            ...
            const msg = `${song.name} ${song.artist || ""}`.trim();
            const result = await API_KUWO.getSongUrlByRid(song.id, quality, msg);
        ...
        // 酷狗音乐使用新的API（通过关键词+序号获取）
        if (song.source === "kugou") {
            // 使用搜索时保存的原始关键词和序号，避免重新构造关键词导致匹配失败
            const keyword = song.searchKeyword || song.name;
            const songIndex = song.searchN || 1;
            // 酷狗API音质映射：999/740→flac(FLAC无损), 320→320, 192→320(酷狗无192), 128→128
            const kgQualityMap2 = { '999': 'flac', '740': 'flac', '320': '320', '192': '320', '128': '128' };
            const kugouQuality = kgQualityMap2[quality] || 'flac';
            const result = await API_KUGOU.getSongUrlByKeyword(keyword, songIndex, kugouQuality);
            if (result && result.url) {
                debugLog(`[酷狗API] 成功获取播放链接: ${result.url.substring(0, 50)}...`);
                return { ...result, usedQuality: quality };
            }
            return null;
        }
        ...
        if (song.source === "qq" || song.source === "apple") {
            ...
            const keyword = song.searchKeyword || song.name;
            const songIndex = song.searchN || 1;
            // QQ音乐音质映射：999→flac, 320→320, 128→128
            const qqQualityMap = { '999': 'flac', '320': '320', '128': '128' };
            const qqQuality = qqQualityMap[quality] || '128';
            const result = await API_QQ.getSongUrlByIndex(keyword, songIndex, qqQuality);
            if (result && result.url) {
    ...
    getLyricWithFallback: async (song) => {
        ...
        // 酷我：优先使用 getSongUrlByRid 时缓存的原生歌词（零额外请求）
        if (song.source === "kuwo") {
            const cachedLrc = kuwoLyricCache.get(String(song.id));
            if (cachedLrc) {
                debugLog(`[歌词] 使用酷我原生歌词缓存: ${song.name}`);
                return { lyric: cachedLrc, apiSource: "kuwo_native" };
            }
        }
                ...
                try {
                    ...
                    if (retryCount >= maxRetries) {
                        ...
                        try {
                            ...
                            if (songId) {
                                const isCached = await AudioCache.isResourceCached(songId, 'cover');
                                if (isCached) {
                                    const cachedCoverUrl = AudioCache.getResourceUrl(songId, 'cover');
                                    debugLog(`[缓存] 从本地缓存加载封面: ${cachedCoverUrl}`);
                                    await loadImageWithTimeout(cachedCoverUrl, loadTimeout);
                                    if (state.currentSong === song && updateBackground) {
                                        setAlbumCoverImage(cachedCoverUrl);
                                        scheduleDeferredPaletteUpdate(cachedCoverUrl, { immediate: true });
                                    }
                                    return;
                                }
                            }
                        } catch (cacheErr) {
                            debugLog(`缓存封面加载失败: ${cacheErr.message}`);
                        }
...
async function performSearch(isLiveSearch = false) {
    ...
    try {
        ...
        let results = [];
        const allSources = ['netease', 'kuwo', 'kugou', 'qq', 'xima'];
        ...
        try {
            if (source === "aggregate") {
                ...
                let platformCounts = { netease: 0, kuwo: 0, kugou: 0, qq: 0, xima: 0 };
                ...
                debugLog(`[聚合搜索] 各平台结果数: 网易=${platformCounts.netease}, 酷我=${platformCounts.kuwo}, 酷狗=${platformCounts.kugou}, QQ=${platformCounts.qq}, 喜马=${platformCounts.xima}`);
                ...
                const sourceOrder = { netease: 0, kuwo: 1, kugou: 2, qq: 3, xima: 4, apple: 3 };
...
// 已移除：可播放性检测相关函数（tryCheckSongPlayable, checkSearchResultsPlayability,
// sortSearchResultsByPlayability, updateSearchResultMuteIcon）
// 原因：检测耗时长不准确，标记不可播的歌曲实际可播放，影响体验
...
function removeFavoriteAtIndex(index) {
    ...
    // 移除收藏时同步删除本地缓存（音频+封面+歌词）
    if (removed) {
        const songId = removed.id || removed.url_id || removed.lyric_id;
        if (songId) {
            AudioCache.remove(songId);
            AudioCache.removeResource(songId, 'cover').catch(() => {});
            AudioCache.removeResource(songId, 'lyric').catch(() => {});
        }
    }
...
async function cacheFavoriteAudio(song, songId) {
    try {
        ...
        if (result && result.url) {
            ...
            if (cached) {
                debugLog(`[缓存] 已缓存收藏歌曲音频: ${song.name}`);
            }
        }

        // 缓存封面图片（先解析代理URL，缓存真实图片）
        const coverUrl = API.getPicUrl(song);
        if (coverUrl) {
            try {
                const picData = await API.fetchJson(coverUrl);
                if (picData && picData.url) {
                    const coverCached = await AudioCache.cacheResource(songId, picData.url, 'cover');
                    if (coverCached) {
                        debugLog(`[缓存] 已缓存封面: ${song.name}`);
                    }
                }
            } catch (e) {
                debugLog(`[缓存] 封面解析失败: ${song.name}`, e.message);
            }
        }
        ...
        if (lyricUrl) {
            const lyricCached = await AudioCache.cacheResource(songId, lyricUrl, 'lyric');
            if (lyricCached) {
                debugLog(`[缓存] 已缓存歌词: ${song.name}`);
            }
        }
    } catch (e) {
...
function clearFavorites() {
    ...
    if (!window.confirm("确定清空收藏列表吗？（所有缓存的音频/封面/歌词将被删除）")) {
        return;
    }
...
async function playSong(song, options = {}) {
    ...
    try {
        ...
        if (!audioData || !audioData.url) {
            ...
            if (song.source === "kuwo") {
            ...
            } else if (song.source === "qq" || song.source === "apple") {
                // QQ音乐使用新API（兼容旧收藏source=apple）
                debugLog(`[播放] QQ音乐使用新API获取音频`);
                const keyword = song.searchKeyword || song.name;
                const songIndex = song.searchN || 1;
                const qqQualityMap = { '999': 'flac', '320': '320', '128': '128' };
                const qualitiesToTry = [selectedQuality, '320', '128'].filter((v, i, a) => a.indexOf(v) === i);
                const results = await Promise.allSettled(
                    qualitiesToTry.map(async (q) => {
                        try {
                            const apiQ = qqQualityMap[q] || '128';
                            const result = await API_QQ.getSongUrlByIndex(keyword, songIndex, apiQ);
                            if (result && result.url) {
                                return { data: result, quality: q };
                            }
                            return null;
                        } catch(e) { return null; }
                    })
                ...
                if (success && success.value) {
                    audioData = success.value.data;
                    state.playbackQuality = success.value.quality;
                    debugLog(`[播放] QQ音乐API成功 (音质: ${success.value.quality})`);
                }
        ...
        // 注意：不将HTTP升级为HTTPS，因为酷我图片服务器(img1.kwcdn.kuwo.cn)不支持HTTPS
        // 封面URL会在 getPicUrl 中通过 buildAudioProxyUrl 走代理，避免混合内容问题
...
async function loadLyrics(song) {
    ...
    // 喜马拉雅没有歌词，直接显示暂无歌词（QQ音乐有歌词，走网易云fallback）
    if (song.source === "xima") {
    ...
    try {
        ...
        try {
            const songId = song.id || song.url_id || song.lyric_id;
            if (songId) {
                const isCached = await AudioCache.isResourceCached(songId, 'lyric');
                if (isCached) {
                    const cachedUrl = AudioCache.getResourceUrl(songId, 'lyric');
                    debugLog(`[缓存] 从本地缓存加载歌词: ${cachedUrl}`);
                    const cachedResp = await fetch(cachedUrl);
                    const lyricData = await cachedResp.json();
                    ...
                    if (lyricText && lyricText.trim()) {
                        ...
                        dom.lyrics.classList.remove("empty");
                        dom.lyrics.dataset.placeholder = "default";
                        debugLog(`[缓存] 缓存歌词加载成功: ${state.lyricsData.length} 行`);
                        return;
                    }
                }
            }
...
const AudioCache = {
    ...
    // 缓存资源
    async cacheResource(resourceId, url, tag) {
        if (!this.swReg || !url) return false;
        try {
            return new Promise((resolve) => {
                const channel = new MessageChannel();
                const timeout = setTimeout(() => resolve(false), 15000);
                ...
                this.swReg.active.postMessage(
                    { type: 'CACHE_RESOURCE', resourceId: String(resourceId), url, tag },
                    [channel.port2]
                );
            });
        } catch (e) {
    ...
    // 检查资源是否已缓存
    async isResourceCached(resourceId, tag) {
        if (!this.swReg) return false;
        try {
            return new Promise((resolve) => {
                const channel = new MessageChannel();
                ...
                this.swReg.active.postMessage(
                    { type: 'CHECK_RESOURCE_CACHE', resourceId: String(resourceId), tag },
                    [channel.port2]
                );
            });
        } catch (e) {
    ...
    // 获取缓存的资源 URL
    getResourceUrl(resourceId, tag) {
        return `/cached-${tag}/${resourceId}`;
    },

    // 删除缓存的资源
    async removeResource(resourceId, tag) {
        if (!this.swReg) return;
        try {
            this.swReg.active.postMessage({ type: 'DELETE_RESOURCE_CACHE', resourceId: String(resourceId), tag });
        } catch (e) { /* 静默 */ }
    }
};
